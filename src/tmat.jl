...
function tmat!(w, mch, decons, a::Vector; n=2, ch="ch11", rs="rs11")
    v = VLO!(w, mch, decons, n=n)

    id = diagm(ones(n) )
    gl = diagm([Gdr!(w, mch[i], a[i]) for i in 1:n] )
    s2 = [2im * mch[i][2] * qcm!(w, mch[i][1], mch[i][2]) / (4π*w) for i in 1:n]
    # sheets
    if rs == "rs11" || "rs1111"
        nothing
    elseif rs == "rs21" || "rs2111"
        gl[1, 1] += s2[1]
    elseif rs == "rs22" || "rs2211"
        gl[1, 1] += s2[1]
        gl[2, 2] += s2[2]
    end
...
pole_LO = function pole_LO!(init_x::Vector, params::Dict, a::Vector; n=2, rs="rs21", μ_tuple::Tuple=(630e0,), ftol=1e-8, xtol=0)
    ...
    if converged(sol) == true
        ...
        return [NaN, NaN]#@. round(sol.zero, digits=2)
    end
end
