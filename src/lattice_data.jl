...
function plot_latt_ener!(ax, latt_pole, latt_pole_err, threshold)
    ...
    [ax.errorbar([i, i, i, i], latt_pole[i], yerr=latt_pole_err[i], fmt="s", c="g") for i in [1, 2, 4]]
    ax.errorbar([3, 3, 3], latt_pole[3], yerr=latt_pole_err[3], fmt="s", c="g")
...
function plot_theory_ener_2level!(ax, pole)
    ds = 0.1
    [ax.scatter([i, i] .+ ds, pole[i], color="#e6091c", marker="^") for i in 1:4]
end
